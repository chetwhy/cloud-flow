...
public class WXPayDomainSimpleImpl implements IWXPayDomain {
    ...
    public synchronized DomainInfo getDomain(WXPayConfig config) {
        DomainStatics primaryDomain = domainData.get(PaymentConstants.DOMAIN_API);
        if (primaryDomain == null ||
                primaryDomain.isGood()) {
            return new DomainInfo(PaymentConstants.DOMAIN_API, true);
        }
        ...
        if (switchToAlternateDomainTime == 0) {
            ...
            return new DomainInfo(PaymentConstants.DOMAIN_API2, false);
        } else if (now - switchToAlternateDomainTime < MIN_SWITCH_PRIMARY_MSEC) {
            DomainStatics alternateDomain = domainData.get(PaymentConstants.DOMAIN_API2);
