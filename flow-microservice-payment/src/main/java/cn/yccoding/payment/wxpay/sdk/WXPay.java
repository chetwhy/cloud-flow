package cn.yccoding.payment.wxpay.sdk;


import java.util.HashMap;
import java.util.Map;

import static cn.yccoding.payment.wxpay.sdk.WXPayConstants.*;
...
/*************************扩充*******************/
    ...
    public Map<String, String> batchQueryComment(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
        String url;
        if (this.useSandbox) {
            url = SANDBOX_BATCHQUERYCOMMENT_URL_SUFFIX;
        }
        else {
            url = BATCHQUERYCOMMENT_URL_SUFFIX;
        }
        String respXml = this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
        return this.processResponseXml(respXml);
    }
    ...
    public Map<String, String> downloadfundflow(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
        String url;
        if (this.useSandbox) {
            url = SANDBOX_DOWNLOADFUNDFLOW_URL_SUFFIX;
        }
        else {
            url = DOWNLOADFUNDFLOW_URL_SUFFIX;
        }
        String respStr = this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs).trim();
        Map<String, String> ret;
        // 出现错误，返回XML数据
